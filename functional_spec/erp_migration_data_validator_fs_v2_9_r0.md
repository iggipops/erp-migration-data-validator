# ERP Migration Data Validator — Functional Specification v2_9_r0

Document history: Not retained prior to this edition. This document (v2_9_r0) marks the starting point for public distribution.
...
| ImportSpec               | Metadata         | No — optional; if absent, all external file columns import as text |
...
The ImportSpec worksheet defines per-column type conversion rules applied during external file import (section 8.6). It is optional — if the sheet is absent or empty, all columns from all external files are imported as raw text, which is the default behavior established in previous versions.

Each row defines conversion behavior for one column of one external file. Columns not covered by any active ImportSpec row are imported as raw text. ImportSpec rows reference external files by their `SheetName` from the ExternalFiles worksheet — the same identifier already used to uniquely identify each external file import unit elsewhere in the framework.
...
- `ExternalFileSheetName` must reference an existing Active = Yes row in ExternalFiles by its SheetName (referential integrity check)
...
| **Column**         | **Check**                                                                                                                                                         |
| Active             | Must not be empty. Must be Yes or No only.                                                                                                                        |
| ExternalFileSheetName          | Must not be empty. Must reference the SheetName of an Active = Yes row in ExternalFiles — this is the referential integrity link between the two sheets.          |
...
- For xlsx files: the worksheet specified in `OriginalSheetName` is loaded from the external file
- For csv files: the full file content is loaded using the `CSVDelimiter` specified in ExternalFiles
- Column names are stripped of leading/trailing whitespace at load time

### 8.6.2 ImportSpec Column Conversion

After loading, the Import Module applies per-column type conversions for any Active = Yes ImportSpec rows matching this file's `SheetName`. ImportSpec rows are independent of each other — each converts exactly one column — so processing order between rows has no effect on the result.
