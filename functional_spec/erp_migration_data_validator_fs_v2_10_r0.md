# ERP Migration Data Validator — Functional Specification v2_10_r0
...
| v2_10_r0     | 9/25/2026 | Color: 5.2's field table already said ValidationRules.Color is required — 8.7.1's shared check said the opposite ("Optional"), and code followed the wrong one. Resolved by making it genuinely required: the Color check moves out of 8.7.1 (no longer shared) into 8.7.3, ValidationRules-only, and is now Required (not empty). EnrichmentRules.Color is unaffected — 5.3 already said optional, and its own check moves to 8.7.2 unchanged in requiredness. For both, the accepted value format is tightened: a built-in named color, a name from config predefined_colors, or a plain 6-digit hex code with no `#` — hex with a `#` prefix or 3-digit shorthand (`#RGB`) is no longer accepted at metadata-validation time, since 8.8.1's fill-writing step needs a value it can use as-is, not one it has to reinterpret. New in 8.8.1: when EnrichmentRules.Color is blank (the only case it still can be, now that the format is otherwise restricted), no fill is applied to the generated column — it's new data, not a flagged issue, so no highlight is a legitimate default. Functional change, no BS impact — this is the first FS revision since the L/r versioning drift was caught; a functional change now correctly bumps L instead of r. |
...
| Color              | Yes          | Highlight color for flagged cells. Must be a built-in named color (RED, YELLOW, GREEN, ORANGE, BLUE, GREY), a name defined in config predefined_colors, or a plain 6-digit hex code with no `#` (e.g. `FF0000`). |
...
| Color              | No           | Optional highlight color applied to all cells in the generated column — both header and data cells. Must be a built-in named color, a name defined in config predefined_colors, or a plain 6-digit hex code with no `#` (e.g. `FF0000`). If blank, no fill is applied (8.8.1). |
...
|----------------------------|--------------------------------------------------------------------------------------------|
| **Column / Check**         | **Rule**                                                                                   |
| Color                      | Optional. If not empty: must be a built-in named color (RED, YELLOW, GREEN, ORANGE, BLUE, GREY), a name defined in config predefined_colors, or a plain 6-digit hex code with no `#` (e.g. `FF0000`, not `#FF0000` or `#F00`). If empty, no fill is applied to the generated column (8.8.1). |
...
|--------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Column / Check**             | **Rule**                                                                                                                                               |
| Color                          | Must not be empty. Must be a built-in named color (RED, YELLOW, GREEN, ORANGE, BLUE, GREY), a name defined in config predefined_colors, or a plain 6-digit hex code with no `#` (e.g. `FF0000`, not `#FF0000` or `#F00`). |
...
- Filled with the rule's Color (both header and data cells) if Color is not empty; left with no fill if Color is empty
